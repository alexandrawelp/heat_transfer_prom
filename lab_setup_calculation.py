...
if complete_subcooling == limited_subcooling:
    raise Exception("invalid input for subcooling, please choose one option")
...
rho_2b = CP.PropsSI("D", "Q", 1, "P", p_c, fluid)
mu_2b = CP.PropsSI("VISCOSITY", "Q", 1, "P", p_c, fluid)
cp_2b = CP.PropsSI("C", "Q", 1, "P", p_c, fluid)


h_2c = CP.PropsSI("H", "P", p_c, "Q", 0, fluid)
s_2c = CP.PropsSI("S", "P", p_c, "Q", 0, fluid)
rho_2c = CP.PropsSI("D", "Q", 0, "P", p_c, fluid)
mu_2c = CP.PropsSI("VISCOSITY", "Q", 0, "P", p_c, fluid)
k_2c = CP.PropsSI("CONDUCTIVITY", "Q", 0, "P", p_c, fluid)
cp_2c = CP.PropsSI("C", "Q", 0, "P", p_c, fluid)
...
rho_sc_water = CP.PropsSI("D", "T",0.5 * (T_sc_out + T_sc_in), "P", 1e5, "REFPROP::water")
cp_methanol = CP.PropsSI("C", "T", 0.5 * (T_evap_in + T_evap_out), "P", 1e5, "REFPROP::methanol")
rho_methanol = CP.PropsSI("D", "T", 0.5 * (T_evap_in + T_evap_out), "P", 1e5, "REFPROP::Methanol")
...
m_sc = Q_storage_sc / (cp_sc_water * delta_T_sc)
m_ws = Q_storage_ws / (cp_ws_water * delta_T_ws)
m_sh = Q_storage_sh / (cp_thermooil * delta_T_sh)

V_methanol = m_methanol / rho_methanol
V_sc = m_sc / rho_sc_water
V_ws = m_ws / rho_ws_water
...
u_sc = m_sc_dot / (rho_sc_water * np.pi * 0.25 * (d_a ** 2 - d_ai ** 2))
u_ws = m_ws_dot / (rho_ws_water * np.pi * 0.25 * (d_a ** 2 - d_ai ** 2))
u_wf = m_dot / (rho_2b * np.pi * 0.25 * d_i ** 2)
...
alpha_ws_i = ht.condensation.Shah(m_dot, 0.5, d_i, rho_2c, mu_2c, k_2c, cp_2c, p_c, p_crit)
