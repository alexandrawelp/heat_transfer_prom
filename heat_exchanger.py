...
def call_HE_co(T_KM_ein, T_W_out, p_KM, p_W, resolution, di, alpha_local, m_dot_1, m_dot_2, fluid_1, fluid_2):
    def heat_exchanger_co_current(x, h, p1, p2, fluid_1, fluid_2, di, alpha_local, m_dot_1, m_dot_2):
        T_AF = fprop.hp_v(h[0], p1, fluid_1)[0]
        T_SF = fprop.hp_v(h[1], p2, fluid_2)[0]
        delta_T = T_AF - T_SF
        dhdx_0 = -np.pi * di * alpha_local * delta_T / m_dot_1
    ...
    h1_ein = fprop.tp(T_KM_ein, p_KM, fluid_1)[2]
    h2_out = fprop.tp(T_W_out, p_W, fluid_2)[2]
    x_var = np.linspace(0, l, resolution)
    h_schaetz = np.zeros((2, resolution))
    h_schaetz[0, :] = h1_ein
    h_schaetz[1, :] = h2_out
    res = solve_bvp(lambda x, h: heat_exchanger_co_current(x, h, p_KM, p_W, fluid_1, fluid_2,
    ...
    T_KM_aus = fprop.hp(res.y[0,-1], p_KM, fluid_1)[0]
    T_W_ein = fprop.hp(res.y[1,0], p_W, fluid_2)[0]
    return T_KM_aus, T_W_ein

def linked_he(T_W9, T_KM1, T_W1_real, p_KM, p_W, fluid_1, fluid_2, resolution):
    T_KM2, T_W8 = call_HE_co(T_KM1, T_W9, p_KM, p_W, resolution, di, alpha_local, m_dot_1, m_dot_2, fluid_1, fluid_2)
...
def get_overall_temperatures(T_KM_ein, T_W_out, p_KM, p_W, resolution, di, alpha_local, m_dot_1, m_dot_2, fluid_1, fluid_2):
    def heat_exchanger_co_current(x, h, p1, p2, fluid_1, fluid_2, di, alpha_local, m_dot_1, m_dot_2):
        T_AF = fprop.hp_v(h[0], p1, fluid_1)[0]
        T_SF = fprop.hp_v(h[1], p2, fluid_2)[0]
        delta_T = T_AF - T_SF
    ...
    if res.message != "The algorithm converged to the desired accuracy.":
        print(res.message)
        raise ValueError("The solver did not converge.")
    T_KM_aus = fprop.hp_v(res.y[0], p_KM, fluid_1)[0]
...
if __name__ == "__main__":
    ...
    T_KM1 = 60 + 273.15
    T_W1_real = 53 +273.15
    T_W9_guess = 55+ 273.15
    resolution = 100

    res = root(linked_he, T_W9_guess, args=(T_KM1, T_W1_real, p_KM, p_W, fluid_1, fluid_2, resolution))
    T_W9 = res.x

    T_KM_R1, T_W_R1 = get_overall_temperatures(T_KM1, T_W9, p_KM, p_W, resolution, di, alpha_local, m_dot_1, m_dot_2, fluid_1, fluid_2)
